...
export const useGlobalState: UseGlobalState = () => {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      const countryCollection = await (
        await fetch("data/countries.geojson")
      ).json();
      const earthquakeCollection = await (
        await fetch("data/earthquakes.geojson")
      ).json();
      setRawData({
        countryCollection,
  ...
  const out = {
    earthquakes: rawData.earthquakeCollection?.features || [],
    countries: rawData.countryCollection?.features || [],
    initialized: rawData.initialized,
  };
...
const useEarthquakes: UseEarthquakes = ({
  earthquakes,
  countries,
  initialized,
}) => {
  const [filters, setFilters] = useState<Filters>(
    useEarthquakesFiltersInitialState
  );

  const earthquakesById = useMemo(
    () =>
      earthquakes.reduce((acc, earthquake) => {
  ...
  const filteredEarthquakes = useMemo(
    () =>
      (!initialized
        ...
        : earthquakes
            .filter((earthquake) =>
              filterByCountry(earthquake, countries, filters.country)
            )
            .filter((earthquake) =>
              filterByMagnitude(earthquake, filters.magnitude)
            )
      ).filter((earthquake) =>
        filterWithDateRange(earthquake, filters.range || {})
      ),
  ...
  const filteredEarthquakeIds = searchedEarthquakes.map(
    (earthquake: any) => earthquake.id
  );

  return {
    filteredEarthquakes: searchedEarthquakes,
    filters,
    setFilters,
    filteredEarthquakeIds,
    earthquakesById,
