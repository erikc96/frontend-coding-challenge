...
export const drawPopup = (map: any, earthquake: Earthquake) => {
  ...
  const text = ReactDOMServer.renderToString(
    <EarthquakeDetails earthquake={earthquake || null} />
  );
  popup.setHTML(text);
  popup.addTo(map);
  // wait 1 second
...
export const Map = () => {
  ...
  const mapData = useMemo(() => {
    return {
      ...earthquakeCollection,
      features: filteredEarthquakes.map((earthquake: Earthquake) => {
  ...
  React.useEffect(() => {
    mapboxgl.accessToken =
      "pk.eyJ1IjoiY3JhaWdvZWoiLCJhIjoiY2xic2F0cnZpMDE1aTNxcGZqZTRubzl2cCJ9.vd4qf2OUBfarhC31e_11jw"; // TODO
    ...
    mapContainer.current.on("load", () => {
      ...
      mapContainer.current.on("click", (e: any) => {
        const features = mapContainer.current.queryRenderedFeatures(e.point, {
        ...
        const earthquake = earthquakesRef.current.find(
          (earthquake: Earthquake) => earthquake.id === feature.properties.id
        );
        if (earthquake) {
